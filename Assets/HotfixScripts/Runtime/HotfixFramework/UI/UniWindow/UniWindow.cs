...
namespace UniFramework.Window
{
    ...
    public static class UniWindow
    {
        ...
        public static void OpenWindowAsync<T>(string location, Action<T> onCreateCallback, params System.Object[] userDatas) where T : UIWindow
        {
            ...
            if (IsContains(windowName))
            {
                ...
                onCreateCallback?.Invoke(window as T);
            }
            else
            {
                UIWindow window = CreateInstance(typeof(T));
                Push(window); //首次压入
                window.InternalLoad(location, OnWindowPrepare, onCreateCallback, userDatas);
            }
        }
